...
<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		if(isset($_POST['save'])){
			...
			$tnote=$_POST['test_notes'];
			$files = $_FILES['image']['name'];
			$image_text = mysqli_real_escape_string($mysqli, $_POST['imgname']);
			$target = "upload/".basename($files);
			$filename = basename($files);
			$mysqli->query("INSERT INTO lab_test(u_case_id,t_name,t_notes,t_img,status) VALUES($caseid,'$tname','$tnote','$filename',1)");
...
<body>
	...
	<script>
		$('.openCase').click(function(){
			...
			$.ajax({
				method: "GET",
				url: "case_model_lab.php?case="+id,
				cache:false,
